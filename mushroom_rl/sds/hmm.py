...
from mushroom_rl.sds.initial import CategoricalInitState
from mushroom_rl.sds.transitions import StationaryTransition
from mushroom_rl.sds.observations import GaussianObservation

from mushroom_rl.sds.utils import ensure_args_are_viable_lists
from mushroom_rl.sds.cython.hmm_cy import forward_cy, backward_cy
...
class HMM:
    ...
    @ensure_args_are_viable_lists
    def filter_reuse_logliks(self, obs, act=None, logliks=None):
        init_ll, trans_ll, obs_ll = self.log_likelihoods(obs, act)
        init_ll = init_ll if logliks else logliks
        trans_ll = np.vstack([logliks[1], trans_ll[-1]])
        obs_ll = np.vstack([logliks[2], obs_ll[0][-1]])
        alpha, _ = self.forward(init_ll, [trans_ll], [obs_ll])
        belief = [np.exp(_alpha - logsumexp(_alpha, axis=1, keepdims=True))
                  for _alpha in alpha]
        trans_ll = trans_ll if obs[0].shape[0] > 1 else logliks[1]
        return belief, (init_ll, trans_ll, obs_ll)
