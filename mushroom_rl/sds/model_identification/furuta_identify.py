...
from option_a2c.sds import rARHMM
from option_a2c.sds.utils import sample_env
...
if __name__ == "__main__":
    ...
    nb_test_rollouts, nb_test_steps = 10, 100
    ...
    trans_mstep_kwargs = {'nb_iter': 100, 'batch_size': 512, 'lr': 5e-4}

    models, lls, scores = parallel_em(nb_jobs=4,
                                      nb_states=nb_states,
    ...
    rarhmm.em(train_obs, train_act, nb_iter=25,
              obs_mstep_kwargs=obs_mstep_kwargs,
              trans_mstep_kwargs=trans_mstep_kwargs,
    ...
    torch.save(rarhmm, open(os.path.abspath(
        os.path.join(__file__, '..',
                     '..')) + '/sds/envs/hybrid/models/' + rarhmm.trans_type + "_rarhmm_furuta.pkl",
                            "wb"))
